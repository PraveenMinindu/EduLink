...
# PHASE 2 / 3 FIXES:
#
#   Fix 1 — future_salary_mid KeyError (crash bug)
#     salary_api.predict_live() now returns future_salary_mid.
#     All three print statements updated to use this field.
#
#   Fix 2 — M7 receives cluster-specific salary
#     Previously reasoning_run() always received top1 salary
#     regardless of which cluster rank was being evaluated.
#     Now each cluster rank receives its own salary result.
#     This corrects the evidence score for rank 2 and rank 3.
#
#   Fix 3 — M6 receives cluster for expanded Jaccard query
#     job_api.get_jobs() now receives the top1 cluster name.
#     This enables the expanded query token set inside job_api
#     to include cluster-specific skill terms, improving
#     Jaccard matching accuracy.
#
# =============================================================
...
def run_pipeline(mcq: dict, writing_text: str,
    ...
    demand  = demand_get(career["roles"])
    ...
    print("[4/7] Salary Estimation (live rate)...")
    # Salary estimated for each cluster separately
    ...
    print(f"      Source: {salary['data_source']}")

    # FIX 1: future_salary_mid now exists in salary_api return dict
    ...
    # FIX 3: pass cluster so job_api can expand Jaccard query tokens
    jobs = job_match(career["roles"], cluster=top1_cluster)
    ...
    # FIX 2: pass cluster-specific salary to reasoning layer
    # Previously only top1 salary was passed for all three cluster evaluations.
    # Now each cluster rank receives its own salary result so the evidence
    # scores for rank 2 and rank 3 are calculated correctly.
    final = reasoning_run(
        career_result  = career,
        writing_scores = writing,
        salary_result  = salary,
        demand_result  = demand,
        salary2_result = salary2,
        salary3_result = salary3,
        education_result = edu,
        job_result       = jobs,
    )
    ...
    # ── Cluster fit score normalisation ──────────────────────
    ...
    cluster_data = {
        top1_cluster: {
            ...
            "future_salary_mid": salary["future_salary_mid"],
            "future_salary_min": salary["future_salary_min"],
            "future_salary_max": salary["future_salary_max"],
            "demand_trend":      demand1["primary_trend"],
        ...
        top2_cluster: {
            ...
            "future_salary_mid": salary2["future_salary_mid"],
            "future_salary_min": salary2["future_salary_min"],
            "future_salary_max": salary2["future_salary_max"],
            "demand_trend":      demand2["primary_trend"],
        ...
        top3_cluster: {
            ...
            "future_salary_mid": salary3["future_salary_mid"],
            "future_salary_min": salary3["future_salary_min"],
            "future_salary_max": salary3["future_salary_max"],
            "demand_trend":      demand3["primary_trend"],
    ...
    report = {
        ...
        # Salary — top1 cluster main report values
        "salary_min":             salary["salary_min"],
        "salary_max":             salary["salary_max"],
        "salary_mid":             salary["salary_mid"],
        "future_salary_mid":      salary["future_salary_mid"],
        ...
        "reasoning_sections":     final.get("reasoning_sections", {}),
...
if __name__ == "__main__":
    ...
    print(f"  Future:       LKR {result['future_salary_mid']:,} (3-5 yrs projected)")
    ...
    for cluster, data in result["cluster_data"].items():
        ...
        print(f"      Salary:  LKR {data['salary_mid']:,} (current)")
        print(f"      Future:  LKR {data['future_salary_mid']:,} (3-5 yrs)")
