...
class _RegisterScreenState extends State<RegisterScreen> {
  ...
  Future<void> _register() async {
    ...
    try {
      ...
      // Step 2: Generate studentId (unchanged)
      final studentId = const Uuid().v4().substring(0, 8).toUpperCase();
      final fullName =
          '${_firstNameCtrl.text.trim()} ${_lastNameCtrl.text.trim()}';

      // Step 3: Save profile to Firestore via backend (unchanged)
      ...
      // Step 4: Save Firebase UID → studentId mapping
      await AuthService.saveUserMapping(studentId, fullName);

      // Step 5: Generate assessmentId for first assessment
      final assessmentId = AuthService.generateAssessmentId(studentId);

      // Step 6: Go to MCQ with assessmentId
      if (mounted)
