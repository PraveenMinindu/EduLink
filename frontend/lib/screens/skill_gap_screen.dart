...
class _SkillGapScreenState extends State<SkillGapScreen> {
  ...
  Widget _buildError() => Center(
    child: Padding(
      ...
      child: Column(
        mainAxisAlignment: MainAxisAlignment.center,
        children: [
          const Icon(Icons.wifi_off, color: AppColors.text3, size: 48),
  ...
  Widget _buildContent(String role) {
    final technical = _skills.where((s) => s['type'] == 'technical').toList();
    final soft = _skills.where((s) => s['type'] == 'soft').toList();
      ...
      child: Column(
        ...
        children: [
          ...
          if (technical.isNotEmpty) ...[
            _sectionLabel("TECHNICAL SKILLS REQUIRED", AppColors.blue),
            const SizedBox(height: 8),
  ...
  Widget _buildStrengthsCard() {
    ...
    if (widget.report.writingAnalytical >= 65)
      strengths.add("Analytical ${widget.report.writingAnalytical.toInt()}%");
    if (widget.report.writingClarity >= 65)
      strengths.add("Clarity ${widget.report.writingClarity.toInt()}%");
    if (widget.report.writingStructure >= 65)
      strengths.add("Structure ${widget.report.writingStructure.toInt()}%");
    if (widget.report.writingConfidence >= 65)
      strengths.add("Confidence ${widget.report.writingConfidence.toInt()}%");
